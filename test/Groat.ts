...
describe("Groat", function () {
  ...
  describe("Adding entries", function() {
    it ("Should add 1 entry", async function() {
    ...
    it("Should add 5 entries from a single account", async function() {
        const {groat, signers} = await loadFixture(deployGroatFixture);
    ...
    it("Should truncate 55 entries to 51 entries from a single account", async function() {
        const {groat, signers} = await loadFixture(deployGroatFixture);
    ...
    it("Should add 51 entries from a single account one at a time", async function() {
        const {groat, signers} = await loadFixture(deployGroatFixture);
        for (let i = 0; i < 51; i++) {
          await groat.depositEth({value: ethers.utils.parseEther("1")});
        }
    ...
    it("Should invalidate own bitmap", async function() {
        const {groat, signers} = await loadFixture(deployGroatFixture);
    ...
    it("Should invalidate another's bitmap", async function() {
      const {groat, signers} = await loadFixture(deployGroatFixture);
  ...
  describe("Deleting entries", function () {
    it("Should add two entries and remove one", async function() {
      const {groat, signers} = await loadFixture(deployGroatFixture);
    ...
    it("Should add an entry, remove it, then add it back", async function() {
      const {groat, signers} = await loadFixture(deployGroatFixture);

      await groat.depositEth({value: ethers.utils.parseEther("1.0")});
      await groat.removeEntries(1);

      var queuePtr = await groat.queuePtr();
    ...
    it("Should add an entry, remove it, then replace it with a different account's entry", async function() {
      const {groat, signers} = await loadFixture(deployGroatFixture);
    ...
    it(`Should correctly replace one person's entry with another's in the internal data structures`, async function () {
      const {groat, signers} = await loadFixture(deployGroatFixture);
    ...
    it("Should remove fewer entries than requested", async function() {
      const {groat, signers} = await loadFixture(deployGroatFixture);
